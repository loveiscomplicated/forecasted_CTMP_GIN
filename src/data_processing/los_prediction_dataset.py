...
class LOSPredictionDataset(Dataset):
    """Canonical view for LOS prediction from admission-only variables."""

    def __init__(self, root: str, do_preprocess: bool = False):
        super().__init__()
        self.root = root
        ...
        self.los_num_classes = int(bundle.los_num_classes)
        ...
        self.y = bundle.los_encoded_tensor.long()
        self.los_raw = bundle.los_raw_tensor.long()

    def __len__(self) -> int:
        return self.x.shape[0]

    def __getitem__(self, idx: int):
        return self.x[idx], self.y[idx], self.los_raw[idx]


def split_los_dataset(
